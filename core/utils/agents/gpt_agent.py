...
class ResumeService:
    ...
    async def create_resume(self, user_id: int, input_resume: Optional[str] = None) -> str:
    ...
    async def edit_resume(self, user_id: int, instruction: str) -> Optional[str]:

        resume_html = await self.redis_client.get(f"resume:html:{user_id}")
        ...
        data = {"html_code": json.loads(resume_html)["html_code"], "user_request": instruction}
